...
MD2Model* MD2Model::load(const char* filename) {
	...
	TImagePtr modelTexture = CImage::CreateImage(imageName);
	MD2Model* model = new MD2Model();
	model->modelTextureImage = modelTexture;
...
void MD2Model::advance(float dt, float x, float z) {
	time += dt;
	if (time < 1000000000) {
		time -= (int)time;
	}
...
void MD2Model::draw() {
	...
	this->modelTextureImage->GetTexture()->Bind();
